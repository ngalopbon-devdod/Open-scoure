...
-- Dung PlayerGui thay CoreGui cho menu chinh (tuong thich executor mobile)
...
local bearlib = {
    Themes = {
        QuangHuy = {
            ...
            ["Color Theme"] = Color3.fromRGB(0, 195, 120),
            ...
            ["Color Discord Border"] = Color3.fromRGB(0, 195, 120),
            ["Color Profile Border"] = Color3.fromRGB(0, 195, 120),
            ["Color Card Border"] = Color3.fromRGB(0, 195, 120),
            ["Color Toggle On"] = Color3.fromRGB(0, 195, 120),
            ["Color Toggle Off"] = Color3.fromRGB(0, 0, 0),
            ["Color Toggle Knob On"] = Color3.fromRGB(255, 255, 255),
            ["Color Toggle Knob Off"] = Color3.fromRGB(0, 195, 120),
            ["Color Toggle Border"] = Color3.fromRGB(0, 195, 120),
            ["Border Thickness"] = 1.5,
            ["UI Border Color"] = Color3.fromRGB(0, 195, 120),
        }
    },
    Info = {
        Version = "1.2.0"
...
function bearlib:MakeWindow(Configs)
    ...
    function Window:CloseBtn()
        local Dialog = Window:Dialog({
            ...
            Options = {
                {"Đóng", function()
                    -- ScreenGui:Destroy() -- da vo hieu hoa
                    if ToggleGui then
                        ToggleGui:Destroy()
                    end
                end},
    ...
    function Window:MakeTab(paste, Configs)
        ...
        function Tab:AddProfile(Configs)
            ...
            local ProfileStroke = InsertTheme(Create("UIStroke", Card, {
                Color = Color3.fromRGB(0, 195, 120),
                Thickness = Theme["Border Thickness"] or 1.5,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            }), "Stroke")
    ...
    task.spawn(function()
        ...
        -- Dung PlayerGui de tuong thich Delta/Fluxus (CoreGui bi block)
        local _pgui = game:GetService("Players").LocalPlayer.PlayerGui
        local _ok = pcall(function() ToggleGui.Parent = game:GetService("CoreGui") end)
        if not _ok then ToggleGui.Parent = _pgui end
        if not ToggleGui.Parent then ToggleGui.Parent = _pgui end
        ...
        _stroke.Color = Color3.fromRGB(0, 195, 120)
        _stroke.Thickness = 3
        _stroke.Parent = ToggleButton
        task.spawn(function()
            while _stroke and _stroke.Parent do
                _stroke.Transparency = 0.05 + math.abs(math.sin(tick()*2.5))*0.55
                task.wait(0.04)
            end
        end)

        -- Ten "Tay Hub" duoi avatar
        local _nameLabel = Instance.new("TextLabel")
        _nameLabel.Size = UDim2.new(0, 80, 0, 18)
        _nameLabel.Position = UDim2.new(0.5, -40, 1, 4)
        _nameLabel.BackgroundTransparency = 1
        _nameLabel.Font = Enum.Font.GothamBold
        _nameLabel.TextSize = 13
        _nameLabel.TextColor3 = Color3.fromRGB(0, 195, 120)
        _nameLabel.Text = "Tay Hub"
        _nameLabel.TextStrokeTransparency = 0.5
        _nameLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
        _nameLabel.Parent = ToggleButton
...
return bearlib
-- ============================================================
-- RAINBOW NHAP NHAY XANH LA <-> XANH DUONG (inject vao BearLib)
-- ============================================================
task.spawn(function()
    task.wait(2)
    ...
    while task.wait(0.04) do
        pcall(function()
            ...
            for _, sg in ipairs(pgui:GetChildren()) do
                if sg:IsA("ScreenGui") and sg.Name ~= "Main" then
                    for _, v in ipairs(sg:GetDescendants()) do
                        pcall(function()
                            if v:IsA("UIStroke") then
                                -- Chi doi UIStroke mau xanh la/duong (khong doi mau den/trang text)
                                local c = v.Color
